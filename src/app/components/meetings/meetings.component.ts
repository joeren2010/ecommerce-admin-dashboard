    ...
    if(this.meetingForm.valid){
      ...
      this.meetingsService.postMeetings(postData).subscribe({
        next: (postRes: any) => {
          ...
          //   window.location.reload(); 
          // }, 1000);  
          //let meetingTopic = postRes.map(meetingTopic => meetingTopic.name)
          for(let i = 0; i < meetingForm.value.length; i++){
            console.log(meetingForm.value[i]);
          }
          console.log(meetingForm.value[2]);
...
export interface Meetings {
  ...
  // ngOnChanges() {
  //   //open meeting modal from calendar date
  //   //this.modalService.open(this.meetingModal);
  //   //this.openModal;        
  ...
  //calenForm: FormGroup = new FormGroup({});
  //updation: boolean = false;


  // //activate meeting modal for everything
  // ngAfterViewInit(){  
  //   this.openModal; 
  //   this.openModal(this.meetingModal); 
  //   this.modalService.dismissAll(this.meetingModal)
